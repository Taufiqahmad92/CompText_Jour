...
#rsw comment - I wanted a total count of distinct articles. Yours gave a count by year
lynch1910 %>% 
  select(filename) %>% 
 distinct(filename, .keep_all = TRUE) %>% 
  count(filename) %>% 
...
```{r}
#rsw comment - library(dplyr) YOU DON'T NEED THIS. you loaded it already with tidyverse
library(dplyr)
```
...
```{r}
#rsw comment: error in this code. it fails when I run it.
...
#rsw comment: see how this works instead
...
#RSW comment - this failed to produce bigrams of the sentences. you produced a list of file names. looks like you missed a crucial step:
wp <- lynch %>% 
  filter(is.na(black_press))
...
#see my method on this answer key. Thanks
https://github.com/wellsdata/CompText_Jour/blob/main/exercises/week5%20KEY%20bigrams_exercise_fall24.Rmd
